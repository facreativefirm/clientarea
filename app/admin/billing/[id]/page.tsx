...
import { ArrowLeft, Printer, Download, Mail, Share2, PlusCircle, CreditCard, CheckCircle } from "lucide-react";
            ...
            <div className="min-h-screen bg-white text-foreground transition-colors duration-300 print:bg-white print:text-black">
                ...
                <main className="pl-0 md:pl-75 pt-20 p-8 flex justify-center print:p-0 print:pt-0 print:pl-0 print:m-0">
                    <div className="w-full max-w-4xl space-y-8 print:w-full print:max-w-none">
                        ...
                        {/* Transactions Section - Admin View */}
                        {invoice.transactions && invoice.transactions.length > 0 && (
                            <div className="bg-card text-card-foreground rounded-[2rem] p-8 shadow-xl border border-border/50 print:hidden mt-8">
                                <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                                    <CreditCard className="text-primary" size={24} />
                                    Transaction History
                                </h3>
                                <div className="space-y-4">
                                    {invoice.transactions.map((txn: any) => (
                                        <div key={txn.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-2xl bg-secondary/10 border border-border/50 gap-4">
                                            <div className="flex items-center gap-4">
                                                <div className={cn(
                                                    ...
                                                    {txn.status === 'SUCCESS' ? <CheckCircle size={20} /> : <CreditCard size={20} />}
                                                </div>
                                                <div>
                                                    <div className="font-bold flex items-center gap-2">
                                                        {formatPrice(txn.amount)}
                                                        <Badge variant={txn.status === 'SUCCESS' ? 'success' : txn.status === 'PENDING' ? 'warning' : 'destructive'} className="text-[10px]">
                                                            {txn.status}
                                                        </Badge>
                                                    </div>
                                                    <div className="text-xs text-muted-foreground">
                                                        {new Date(txn.createdAt).toLocaleDateString()} via {txn.gateway}
                                            ...
                                            {txn.status === 'SUCCESS' && (
                                                <div className="flex gap-2 w-full sm:w-auto">
                                                    <Button
                                                        ...
                                                        onClick={() => window.open(`${process.env.NEXT_PUBLIC_API_URL || '/api'}/finance/transactions/${txn.id}/receipt/download?token=${getSessionToken()}`, '_blank')}
                                                    >
                                                        <Download size={14} />
                                                        Receipt
                                                    </Button>
                                                    <Button
                                                        ...
                                                        size="sm"
                                                        className="gap-2 flex-1 sm:flex-none"
                                                        onClick={() => window.open(`/client/transactions/${txn.id}/print`, '_blank')}
                                                    >
                                                        <Printer size={14} />
                                                        Print
                                                    </Button>
